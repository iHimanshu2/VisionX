...
def Action(phrase):
    ...
    if 'help navigation' in phrase:
        say('Sending SOS now')
        os.system("python3 gps.py")
        
    if 'grocery mode' in phrase:
        say('Activating Grocery Mode')
        os.system("python3 /home/pi/VisionX/foundGrocery.py")
        fp = open("/home/pi/VisionX/foundGrocery.txt", 'r')
        lines = fp.readlines()
        fp.close()
        while (not lines):
            fp = open("/home/pi/VisionX/foundGrocery.txt", 'r')
    ...
    if 'close object detection' in phrase:
        say('Turning off Object Detection')
        # time.sleep(10)
        os.system("./closeObjectDetector")
        #os.system("source tflite1-env/bin/activate")
        #os.system("python3 TFLite_detection_webcam.py --modeldir=Sample_TFLite_model --edgetpu")
        #subprocess.call(["shutdown -h now"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
    if 'recognize text' in phrase:
        say('Hold still, recognizing text')
        fp = open("/home/pi/textrecognitionstatus.txt", "w")
        fp.write("True")
        fp.close()
        os.system("tesseract /home/pi/VisionX/text.png recognizedText")
        time.sleep(10)
        fp = open("recognizedText.txt", "r")
        lines = fp.readlines()
        data = ''
        for i in lines:
            data = data + i.strip() + '. '
    ...
    if 'open object detection' in phrase:
        say('Turning open Object Detection')
        # time.sleep(10)
        os.system("./openObjectDetector")
        
    if 'what do you see' in phrase:
        fp = open("/home/pi/VisionX/allobject.json", "r")
        data = json.load(fp)
        fp.close()

        if (len(data['objects']) == 0):
            say("Sorry, I was unable to recognize anything")
        else:
            objects = 'I see '
            for i in range(len(data['objects'])):
                objects = objects + data['objects'][i]['name'] + ' in the ' + data['objects'][i]['position'] + ', '
            say(objects)


    if 'list' in phrase:
        grocery_item = ''
        grocery_command = phrase.partition('list')[2]
        
        if 'read' in phrase:
            f = open("/home/pi/VisionX/groceryList.txt", "r")
            items = f.readlines()
            text = "Your list has "
            for i in items:
                #say(i.strip())
                text = text + i.strip() +", "
            if text != "Your list has ":
                say(text)
                

                
        if 'add' in phrase:
            grocery_item = grocery_command.partition('add ')[2]
            if grocery_item.strip(): # Check to see if word is not empty
                f = open("/home/pi/VisionX/groceryList.txt", "a+")
                f.write(grocery_item + "\n")
                f.close()
                say("Okay, I have added " + grocery_item)
            else:
                say("Please repeat the command")


        elif 'remove' in phrase:
            grocery_item_delete = grocery_command.partition('remove')[2]
            fp = open("/home/pi/VisionX/groceryList.txt", "r")
            grocery_items = []
            line = fp.readline()
            while line:
                line.strip()
                if not grocery_item_delete.strip() in line:
                    grocery_items.append(line.strip())
                line = fp.readline()
            fp.close()
            fp = open("/home/pi/VisionX/groceryList.txt", "w")
            for i in grocery_items:
                fp.write(i + "\n")
            fp.close()
            say("Removing" + grocery_item_delete + "from the list")
