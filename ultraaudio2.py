...
def find_distance():
    try:
          ...
          while GPIO.input(front_echo)==0:
                pulse_start_time_front = time.time()
          while GPIO.input(front_echo)==1:
                pulse_end_time_front = time.time()
          ...
          pulse_duration_front = pulse_end_time_front - pulse_start_time_front
          distance_front = round(pulse_duration_front * 17150, 2)
          print ("Distance_Front:", distance_front, "cm")


          sound_left = pygame.mixer.Sound("left_back.wav")
          sound_right = pygame.mixer.Sound("right_back.wav")
          sound_front = pygame.mixer.Sound("left_back.wav")

          if (distance_left < 10 and distance_right < 10) and distance_front < 10:
              sound_left.play()
              sound_right.play()
              sound_front.play()
              time.sleep(0.7)
